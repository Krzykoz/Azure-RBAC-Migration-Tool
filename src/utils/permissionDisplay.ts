 ...
 * Both the live view (`PermissionVisualizer`) and the static HTML export
 * (`htmlExport`) consume these so the two never drift apart.
 */
 ...
 * the missing/covered/excess(+privileged) branching that the live
 * `PermissionVisualizer` and the static `htmlExport` both need; each maps the
 * returned variant to its own class names (Tailwind vs. CSS) and icons.
 */
