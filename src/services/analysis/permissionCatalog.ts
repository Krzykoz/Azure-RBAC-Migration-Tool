...
/**
 * A `PermissionCatalog` is the abstraction the analysis engine depends on
 * instead of module-global singletons. It owns the legacy→RBAC mapping and the
 * universe of known RBAC actions, so the matching/coverage/strategy logic can be
 * exercised in isolation with a hand-built catalog (Dependency Inversion).
 */
 ...
 * existing callers need no wiring; tests can inject their own catalog instead.
 */
